...
dbMyles = create_engine('sqlite:///' + os.path.join(basedir, 'dbs/app_myles.db'))
dbAlex = create_engine('sqlite:///' + os.path.join(basedir, 'dbs/app_alex.db'))
dbShane = create_engine('sqlite:///' + os.path.join(basedir, 'dbs/app_shane.db'))
...
for session in [dbMylesSession, dbAlexSession, dbShaneSession]:
  ...
  while True:
    for b in businessPage.items:
      ...
      sys.stdout.flush()
      biz = Business( b.unique_id, b.name, b.address, b.description, b.website, b.business_type, b.features_dict)
