...
"""Cleans business types.
Removes common words that add no description.
"""
...
def clean_business_types():
    ...
    for k in business_types_dict.keys():
        ...
        s = []
        for st in strs:
            if st not in [None, 'agency', 'place', 'food', 'point_of_interest', 'establishment', 'sublocality', 'route',
                          'real', 'political', 'of', 'or', 'local', 'locality', 'intersection', '1']:
                s += [st]
        if len(s) == 0:
            business_types_dict[k] = ''
        else:
